...
export function generarReportePorArea(areaNombre, carpetasDelArea, opts = {}) {
  ...
  // Ampliado a 7 niveles de profundidad para mantener consistencia con Excel
  ...
  function empujarFila(c, nivelVisual) {
    ...
    const sangriaIzq = 2 + Math.max(0, (nivelVisual || 1) - 1) * 4; 
  ...
  function empujarSubEncabezado(nombre, nivel) {
    ...
    const sangriaIzq = 2 + Math.max(0, nivel - 1) * 4;
    body.push([
      {
        content: `➤  ${nombre}`,
        colSpan: 4,
        styles: {
          fillColor: color,
          textColor: nivel === 1 ? [255, 255, 255] : [74, 45, 15],
          fontStyle: "bold",
          fontSize: nivel === 1 ? 11 : 9.5,
          halign: "left",
  ...
  // Ampliado a 6 niveles intermedios para cubrir hasta 7 niveles totales de jerarquía
  ...
  function agruparRecursivo(items, nivelIdxRuta, nivelVisual) {
    ...
    for (const entrada of entradas) {
      if (entrada.esGrupo) {
        empujarSubEncabezado(entrada.key, nivelVisual);
  ...
  for (const especialidad of ordenGrupos) {
    empujarSubEncabezado(especialidad.toUpperCase(), 1);
