...
export function generarReportePorArea(areaNombre, carpetasDelArea, opts = {}) {
  ...
  for (const especialidad of ordenGrupos) {
    ...
    // --- Sub-agrupar dentro de la especialidad por la carpeta intermedia (nivel 2) ---
    // Si el ítem está directo dentro de la especialidad (sin carpeta intermedia),
    // se muestra igual que antes, sin sub-encabezado de más.
    const itemsEspecialidad = grupos[especialidad];
    const subgrupos = {};
    const ordenSubgrupos = [];
    for (const c of itemsEspecialidad) {
      const partesItem = (c.ruta || c.nombre || "").split(" / ").filter(Boolean);
      const nivel2 = partesItem.length > 3 ? partesItem[2] : null;
      const clave = nivel2 || "__directo__";
      if (!subgrupos[clave]) {
        subgrupos[clave] = [];
        ordenSubgrupos.push(clave);
      }
      subgrupos[clave].push(c);
    }
    ordenSubgrupos.sort((a, b) => {
      if (a === "__directo__") return -1;
      if (b === "__directo__") return 1;
      return a.localeCompare(b, undefined, { numeric: true, sensitivity: "base" });
    });

    for (const clave of ordenSubgrupos) {
      if (clave !== "__directo__") {
        body.push([
          {
            content: `   ↳ ${clave}`,
            colSpan: 4,
            styles: { fillColor: [222, 232, 235], textColor: [40, 60, 65], fontStyle: "bold", fontSize: 7.5, halign: "left" },
          },
        ]);
      }
      for (const c of subgrupos[clave]) {
        const estado = c.estado || "incompleta";
        const nombreMostrado = c.nombre || (c.ruta || "").split(" / ").pop() || "-";
        body.push([
          String(n++),
          clave !== "__directo__" ? `     ${nombreMostrado}` : nombreMostrado,
