...
export function generarReportePorArea(areaNombre, carpetasDelArea, opts = {}) {
  ...
  function empujarSubEncabezado(nombre, nivel) {
    // Se ve más clarito y con más sangría mientras más profundo el nivel (2, 3 o 4)
    const tonos = [
      [222, 232, 235], // nivel 2
      [232, 238, 240], // nivel 3
      [240, 244, 246], // nivel 4
    ];
    const color = tonos[Math.min(nivel - 2, tonos.length - 1)];
    const sangria = "   ".repeat(nivel - 1);
    body.push([
      {
        content: `${sangria}↳ ${nombre}`,
        colSpan: 4,
        styles: { fillColor: color, textColor: [40, 60, 65], fontStyle: "bold", fontSize: 7.5, halign: "left" },
      },
    ]);
  }
  ...
  // y el ítem final = nivel5 → 5 niveles totales, como se pidió). Todo ordenado
  // numéricamente en cada nivel (01,02,03... o Anexo1,Anexo2... lo que sea).
  const NIVEL_MAX_INTERMEDIO = 4; // índice de partes[] hasta donde se agrupa (2,3,4)
  function agruparRecursivo(items, nivelIdx) {
    if (nivelIdx > NIVEL_MAX_INTERMEDIO) {
      const ordenado = [...items].sort((a, b) => comparaNatural(a.nombre, b.nombre));
      ordenado.forEach(empujarFila);
      return;
    }
    ...
    for (const key of orden) {
      if (key === "__directo__") {
        ...
        empujarSubEncabezado(key, nivelIdx);
        agruparRecursivo(subgrupos[key], nivelIdx + 1);
      }
    }
  }
  ...
  for (const especialidad of ordenGrupos) {
    ...
    agruparRecursivo(grupos[especialidad], 2);
  }
