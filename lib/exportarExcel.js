...
/**
 * Genera y descarga un reporte en Excel (.xlsx) con jerarquía uniforme de hasta 7 niveles,
 * sangrías escalonadas, degradados de color y ordenamiento natural.
 */
export async function generarReporteExcelPorArea(
  ...
  // --- Soporte completo para hasta 7 niveles jerárquicos ---
  ...
  const NIVEL_MAX_INTERMEDIO = 6; // 6 niveles intermedios + 1 hoja = 7 niveles totales
  function agruparRecursivo(items, nivelIdxRuta, nivelVisual) {
    ...
    for (const c of items) {
      const partes = (c.ruta || c.nombre || "").split(" / ").filter(Boolean);
      // CORREGIDO: Se usa >= para garantizar que carpetas terminales actúen de manera uniforme
