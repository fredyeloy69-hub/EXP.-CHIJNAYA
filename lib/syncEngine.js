...
function computeEstado(folderNode) {
  ...
  const tieneEditable = tieneWord || tieneExcel;

  const tieneAmbos = tieneWord && tieneExcel;
  let nombresEditables = "";
  if (tieneAmbos) nombresEditables = "Word y Excel";
  else if (tieneWord) nombresEditables = "Word";
  else if (tieneExcel) nombresEditables = "Excel";
  ...
  if (tienePdf && !tieneEditable) {
    return {
      estado: "incompleta",
      detalle: `Falta Word o Excel — solo tiene ${pdfFiles.length} PDF`,
      tienePdf,
      tieneEditable,
      pdfCount: pdfFiles.length,
      extensionesEditables,
  ...
  // hay archivos, pero ninguno es PDF ni Word/Excel (ej. imagenes, dwg, etc.)
  return {
    estado: "incompleta",
