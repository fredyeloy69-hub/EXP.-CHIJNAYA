...
function computeEstado(folderNode, override) {
  ...
  const tieneSoloRespaldo = respaldoFiles.length > 0 && otrosFiles.length === 0;
  ...
  // Cuántas "unidades editables" distintas hay (word cuenta cada archivo, dwg/rvt cuentan como 1 aunque tengan varios archivos)
  const unidadesEditables = wordFiles.length + excelFiles.length + (tieneDwg ? 1 : 0) + (tieneRvt ? 1 : 0);
  ...
  const mensajeOtro = tieneSoloRespaldo
    ? `Falta el archivo original (DWG/RVT) — solo tiene el respaldo (.bak)`
    : `Falta PDF y Word/Excel/DWG/RVT — solo tiene ${otrosLabel}${respaldoFiles.length > 0 ? " + respaldo(s) .bak" : ""}`;
  return {
    estado: "incompleta",
    detalle: mensajeOtro,
    tienePdf,
